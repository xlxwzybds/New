...
def execute_gdp_power(core_num):
    ...
    ## I should find the new logFILE for Temperature and Power
    ...
    # You should know where the Files is !
    #T_c = np.loadtxt('./temperature_core.trace',skiprows=1) # previous power consumption
    #P_k = np.loadtxt('./power_core.trace',skiprows=1) # current temperature
    #
    # You should know that :
    # In GDP, power is [mem_power , core_power]
    # In CoMeT 3D_16core16bank is [core_power , mem_power]
    ###############
    # 1. XJY change here
    ###############
    T_c = np.loadtxt('./combined_insttemperature.trace',skiprows=1) # previous power consumption , 32 x 1
    P_k = np.loadtxt('./combined_instpower.trace',skiprows=1) # current temperature, 32 x 1
    num = 16
    T_cc = T_c;
    P_kk = P_k;
    T_c = np.hstack((T_cc[num:] , T_cc[:num] ))
    P_k = np.hstack((P_kk[num:] , P_kk[:num] ))

    # formulate the static power vector: in hotsniper, every core (active or not) has the same static power
    P_s = np.full((A.shape[0],), inactive_power) ## the [32x1] vector should be cut into [(0,8)x1] vector , onlycore should use static power
    ...
    for power in P:
        ...
        file_power.write(str(float(power))+' ')
    file_power.close()
